...
export function SharedProjectPage({ shareToken }: { shareToken: string }) {
  ...
  const loadProject = useCallback(async (code?: string) => {
    const next = await loadSharedProject(shareToken, code);
    setProject(next);
    return next;
  }, [shareToken]);

  useEffect(() => {
    const savedCode = window.sessionStorage.getItem(`wizerview:project-access:${shareToken}`) ?? undefined;
    void loadProject(savedCode);
  ...
  if (project.requiresAuthentication) {
    return <main className="min-h-screen bg-canvas px-4 py-6 text-text lg:px-8"><section className="mx-auto mt-12 max-w-md rounded-lg border border-border bg-surface p-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-text-subtle">WizerView</p><h1 className="mt-2 text-xl font-semibold">Sign in to view this project</h1><p className="mt-2 text-sm leading-6 text-text-muted">Project access is managed through an authenticated client invitation. Sign in with the email address your creator invited.</p><Link href="/client/sign-in" className="mt-4 inline-block rounded-md bg-brand px-3 py-2 text-sm font-semibold text-white">Sign in to client portal</Link></section></main>;
  }

  if (project.requiresAccessCode) {
    return <main className="min-h-screen bg-canvas px-4 py-6 text-text lg:px-8"><section className="mx-auto mt-12 max-w-md rounded-lg border border-border bg-surface p-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-text-subtle">WizerView</p><h1 className="mt-2 text-xl font-semibold">Enter project access code</h1><p className="mt-2 text-sm text-text-muted">This project is protected by the creator.</p><form onSubmit={(event) => { event.preventDefault(); setAccessMessage(null); void loadProject(accessCode).then((next) => { if (!next || next.requiresAccessCode) { setAccessMessage('That code could not be verified.'); return; } window.sessionStorage.setItem(`wizerview:project-access:${shareToken}`, accessCode); }); }} className="mt-4"><input required type="password" value={accessCode} onChange={(event) => setAccessCode(event.target.value)} placeholder="Access code" className="w-full rounded-md border border-border px-3 py-2"/><button className="mt-3 rounded-md bg-brand px-3 py-2 text-sm font-semibold text-white">Open project</button></form>{accessMessage ? <p className="mt-3 text-sm text-text-muted" role="status">{accessMessage}</p> : null}</section></main>;
  }
